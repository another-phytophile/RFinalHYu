...
```{r scatter}
scatter <- ggplot(data=House, aes(x = BsmtUnfSF, y = SalePrice, color=BsmtCond)) +
  geom_point()
scatter
```
...
```{r panel}
scatterfacet <- ggplot(data=House, aes(x = BsmtUnfSF, y = SalePrice, color=BsmtCond)) +
  geom_point() + 
  facet_wrap(~LotShape)
scatterfacet
```
...
mlrhouse <- lm(SalePrice ~ BsmtUnfSF + OverallQual, data=House)
